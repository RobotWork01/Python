...
■バッチ処理（オフライン作業）・・・・基本的に黒い画面（コマンド入力）での操作、他のシステムのバックグラウンド作業となります。<br/>
＜他の開発言語と同じく、ファイル参照・更新やデータベース参照・更新をバックグラウンドで行う＞<br/>
```
・ExcelからPythonを呼び出す
...
■フロントエンド処理（オンライン作業）<br/>
＜Pythonの豊富なライブラリでGUIアプリを作成＞<br/>
```
...
### pythonは、結局、開発環境(Pythonプログラムファイルを作りたい場所、自分のPCなど)に色々とインストールしなければ開発出来ないのだ
```
...
この辺りが開発する前に戸惑うポイントになりました。
インストールは、１度出来てしまえば２度はしないのでどうやってインストールしたのかも書き留めていないと忘れます。
...
＜＜実行結果＞＞
<img width="940" height="790" alt="image" src="https://github.com/user-attachments/assets/783c00dc-5403-4f39-b217-d2bcea2e9ef2" />

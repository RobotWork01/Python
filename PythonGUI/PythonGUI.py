#---------------------------------------------------------------#
# RobotWork01
#     FreeSimpleGUI(フリー・シンプル・ジーユーアイ)で作った
#     一番簡単なGUI(ジーユーアイ)
#
#
# 参考：https://zenn.dev/unr_tech_lab/articles/882fdddb2f8028
#      FreeSimpleGUI(フリー・シンプル・ジーユーアイ)の使い方・クイックスタートガイド
#
# 環境構築方法
# py -3.9 -m pip install pyinstaller
# pip install freesimplegui
#
# コンパイル＆exeファイル「PythonGUI.exe」作成方法
# py -m  PyInstaller PythonGUI.py --onefile
#
# 留意点
...
window = sg.Window(
 ...
 #no_titlebar=True,                  # ウィンドウタイトルを非表示
 #grab_anywhere=True,                # ウィンドウドラッグ操作可能にする
...
while True:
  ...
  # ウィンドウの×ボタンが押された場合、ループを終了
  if event == sg.WINDOW_CLOSED:break
  
  # 終了ボタンが押された場合、ループを終了
  if event == '-BEnd-':
    if sg.popup_yes_no('本当に終了しますか？　　　　　', title='確認メッセージ') == "Yes":break # 留意点：メッセージ文字数15桁必要
